...
const getDir = async (path: string) =>
  (await stat(path)).isDirectory() ? path : dirname(path)
...
const getPaths = async (
  ...
  for (const comp of componentsDir) {
    ...
    const pattern =
      ((typeof comp === 'object' ? comp : {}).pattern as string | undefined) ??
      defaultPattern
    resolvedComponents.push(join(resolvedPath, pattern))
  }

  const paths = [`${pages}/**/*.vue`, `${layouts}/**/*.vue`]
    .concat(resolvedComponents, additionalPaths)
...
export const findComponents = async (
  ...
  for (const path of paths) {
    ...
    if (template.src) {
      template.content = await readFile(
        resolve(join(await getDir(path), template.src)),
        { encoding: 'utf8' }
      )
    }
  ...
  const validTags = Object.keys(Components)
    ...
    .map((name) => 'B' + name)
  const filteredComponents = Array.from(matchedTags).filter((name) =>
    validTags.includes(name)
  )
...
export const findProgrammaticComponents = async (
  config: NuxtConfig,
  resolve: (path: string) => string
) => {
  ...
  for (const path of paths) {
    ...
    const programmaticMatches = Array.from(
      script.content.matchAll(programmaticRegex)
    )
      .map((match) => match[1])
      .map(capitalize)
